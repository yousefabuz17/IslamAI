...
from typing import NamedTuple, Union, Literal, Type
# import rasa
...
class QuranAPI(BaseAPI, metaclass=SingletonMeta):
    def __init__(self, _holder: Type=None):
        self.url = BaseAPI.config.quran_url
        self.headers = {
            "X-RapidAPI-Key": BaseAPI.config.quran_api,
            "X-RapidAPI-Host": BaseAPI.config.quran_host
        }
    ...
    def best_match(self, string, **kwargs):
        #?> Add a check ratio method
        surah_names = self._list_surahs
        values_ = kwargs.get('values_', surah_names)
...
class HadithAPI(BaseAPI, metaclass=SingletonMeta):
    def __init__(self, _holder: Type=None):
        super().__init__(_holder)
        self.url = BaseAPI.config.hadith_url
        self.headers = None
        self._holder = _holder
    ...
    async def _extract_urls(self, **kwargs):
        async def _parser(contents):
            for book, link in contents.items():
                with open(self.path / f'book_{book}.json', mode='w', encoding='utf-8') as file2:
                    hadith_json = await self._holder._request('', slash=False, url=link)
        ...
        json_file = await self._holder._request('', slash=False, url=self.url)
        ...
        if _:
            ...
            return contents
        if parser:
            json_file = self._holder._load_file(path=self.path, name='hadith_api_links', mode='r')
    ...
    async def get_hadith(self, **kwargs):
        ...
        if author:
            author = self._holder.best_match(author, values_=book_authors)
...
class IslamFacts(BaseAPI, metaclass=SingletonMeta):
    facts = set()
    allah_names = dict()
    
    def __init__(self, _holder=None):
        super().__init__(_holder)
    ...
    def _update_facts(cls, facts: set):
        file2 = cls._holder._load_file(path=cls.path, name='islam_facts', mode='r')
        ...
        if len(self.facts) == 0:
            ...
            while len(self.facts) <= 18:
                for _ in range(limit):
                    html_content = await self._holder._request('', slash=False, url=self.url.islam_facts)
    ...
    async def _extract_contents(self, **kwargs):
        ...
        main_page = await self._holder._request(endpoint=endpoint, slash=slash, url=url)
    ...
    async def _get_name_contents(self, export=False):
        ...
        def extract_name_data(html_contents):
            contents = {
                'transliteration': re.sub(r'[()]', '', html_contents[0][0].text),
